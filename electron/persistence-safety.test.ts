...
describe('Electron persistence safety', () => {
  ...
  test.each([1, 7, 23, 89, 511, 2026, 8191, 65537])(
    ...
    async (seed) => {
      ...
      const notify = () =>
        harness.watcherCallbacks[0]?.('rename', random() < 0.2 ? null : 'todo.md');
      ...
      for (let edit = 0; edit < 300; edit++) {
        ...
        const pending = save(
          { sender },
          { baseHash: disk.hash, content: `Edit ${edit}\n`, path: disk.path },
        );
        notify();
    ...
    (platform) => {
      ...
      for (const window of windows) {
        expect(window.webContents.send).toHaveBeenLastCalledWith(
          'meetings:system-accent-changed',
          '#a070ccff',
        );
      }
      systemPreferences.getAccentColor.mockReturnValue('b080ddff');
      nativeTheme.emit('updated');
      expect(windows[0].webContents.send).toHaveBeenLastCalledWith(
        'meetings:system-accent-changed',
        '#b080ddff',
      );
      harness.appEvents.get('will-quit')!();
      expect(systemPreferences.unsubscribeNotification).toHaveBeenCalledTimes(
        platform === 'darwin' ? 1 : 0,
      );
    },
  );
