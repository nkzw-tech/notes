...
describe("DocumentPalette creation flow", () => {
  test("chooses a type and submits the entered name", async () => {
    const onClose = vi.fn();
    ...
    await act(async () => {
      root.render(
        <DocumentPalette
          activeDocument={createMeetingDocument(
            {
              content: "# ToDo\n",
              hash: "todo",
              mtimeMs: 1,
  ...
  test("offers interview completion only for the active interview and confirms deletion", async () => {
    ...
    expect(container.textContent).toContain(
      "Permanently delete interviews/15-candidate-name.md",
    );

    await act(async () => findCompleteButton()?.click());
    expect(onCompleteInterview).toHaveBeenCalledWith(
      "interviews/15-candidate-name.md",
    );
  });
  ...
  test("offers document deletion and requires a confirmation step", async () => {
    ...
    expect(container.textContent).toContain(
      "Permanently delete docs/design-notes.md",
    );
