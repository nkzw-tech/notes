...
beforeEach(() => {
  ...
  window.meetings = {
    saveDocument: vi.fn(async (request: { baseHash: string; content: string }) => {
      if (request.baseHash !== disk.hash) return { document: disk, status: "conflict" };
      disk = {
        ...disk,
...
test.each([1, 17, 73, 2026])(
  ...
  async (seed) => {
    ...
    while (elapsed < 360_000) {
      const step =
        random() < 0.03 ? 500 + Math.floor(random() * 3_000) : 20 + Math.floor(random() * 120);
    ...
    expect(edits).toBeGreaterThan(1_000);
  },
  120_000,
);
